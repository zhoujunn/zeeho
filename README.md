...
- **传感器**：座垫锁、总里程、本月里程、本月骑行时长、本月均速、蜂窝信号、联网服务到期、充电功率 / 电压 / 电流。整车锁定（`VehicleLock_S` 原始值）默认关闭，需要时再手动打开
- **二进制传感器**：充电中、在线
- **按钮**：短按寻车、高声寻车
...
不要把「整车锁定」当成可以下发的锁，也不要用它判断 App 有没有解开。抓包里云端解锁后这个值可以一直是 `0`。需要看状态时以「车锁」为准。
